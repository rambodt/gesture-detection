...
// EVAL_MODE 1 = bypass password state machine, log numbered trials for
...
void setup() {
  ...
  // Wait for Serial Monitor, but don't hang forever: if the board is powered
  // without a laptop attached, continue after 3 s so the LCD still works.
...
#if EVAL_MODE
  Serial.println("EVAL_MODE = 1 (trial logging, password FSM bypassed)");
#endif
  Serial.println("==============================");
  Serial.println();
...
void processGesture(const char* gesture) {
  // PATCH 2 (do not lose in merges): Idle must never advance or reset the
  // password. Without this, an Idle classification mid-sequence fails the
  // strcmp against the expected gesture and triggers PASSWORD INCORRECT.
...
void loop() {
  ...
  evalTrial++;
  Serial.println();
  Serial.print("EVAL TRIAL ");
  Serial.print(evalTrial);
  Serial.print(" -> ");
  if (bestGestureIndex >= 0 && highestConfidence >= GESTURE_CONFIDENCE) {
    Serial.print(result.classification[bestGestureIndex].label);
  } else {
    Serial.print("NO_CONFIDENT");
  }
  Serial.print(" (");
